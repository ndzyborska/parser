...
int main(int argc, char** argv) {
  ...
  if (argc > 2) {
     if (strcmp(argv[2], "val") == 0 ) {   /* Turn off SDL for valgrind*/
      p -> valgrind = true;
     }
...
void insert_code(Program* p, FILE* fp) {
  ...
  while (fscanf(fp, "%s", p->code[x++]) == 1 && x < MAXLEGNTH)  {
    p->line_index[x] = line;
    if ((c = fgetc(fp)) == '\n') {
      p->line_index[x] = line;        /* Line counter to source errors*/
      line++;
    }
  }
}
...
bool all_numbers(char* str) {    /* To ensure VARNUM variables don't have invalid characters*/
...
void create_var(Program* p, char* name, double value) {
  ...
  if (var_check(name, p) == true) {         /* If variable exists, change variable instead */
  ...
  var -> next = p -> vars;                /*Enter variable at start of list*/
  p -> vars = var;
  if (p -> vars == NULL) {
...
void change_var(Program* p, char* name, double new_value) {
  ...
  while(tmp != NULL) {
    ...
    if(strcmp(tmp->name, name) == 0) {       /*Scan through list and change value*/
      tmp-> val = new_value;
      return;
    }
    tmp = tmp -> next;
...
void FD(char* string, Program* p) {
  ...
  if (in_range(number, MAXMOVEMENT)) {
    ...
    if (p->valgrind == false)  {
      art_attack(&(p->sw), p->co);                       /* Render draw line using found Coordinates*/
    }
    return;
  }
...
void RT(char* string, Program* p) {
  ...
  if (in_range(number, MAXTURN)) {
    p -> co->angle = p-> co->angle + number * RADIAN_CONV;       /* Change the angle relative to previous*/
    return;
  }
...
void DO(char* string, Program* p) {
  ...
  loop_start = varnum(p->code[p->cw], p);             /* Interpret start of the loop and save as variable */
  ...
  loop_limit = varnum(p->code[p->cw], p);            /* Interpret end of the loop */
  loop_cw = p->cw;                                   /* Save current word before jumping in the loop*/

  for (i = loop_start; i <= loop_limit; i++)   {
    change_var(p, string, i);
    start(p);
    save_word = p->cw;
    p -> cw = loop_cw;                               /* Change the current word back to the start of loop */
  }
  p -> cw = save_word;                               /* Once out of loop, bring current word back to end of loop */
}
...
void SET(char* string, Program* p) {
  ...
  if (!string_match(p, ":="))  {
   ...
   create_var(p, string, Polish(p, &s));            /*Create varible using the return from polish notation functions*/
...
void CHANGE(Program* p) {
  /*Jump coordinates for SDL display*/
  double x, y;
...
void find_coord(Coordinates* co, double number) {
  ...
  co->y = co->y_new;
   ...
   /*Trig using prviously declared angles and length to find new coordinates*/
...
double Polish(Program* p, Stack* s)   {
  ...
  if(strcmp(p->code[p->cw], ";") == 0) {
     /* End of polish notation */
    if (s->count == 1 ) {
    /* If only 1 value left in stack, reset, and return result*/
      s -> count = 0;
      return s->result;
    }
    else {
      error("Incorrect format of Polish notation", p);
       /* Else, incorrect format */
    }
  }

  if (operator(p->code[p->cw], s)) {
    /*If operater, pop 2, push result*/
      operate(s);
      increment(p);
  }
  ...
  s -> result = Polish(p, s);   /*Recurse through*/
  ...
  if(s->result >=0 ) {
    ...
    error("Incorrect format of Polish notation\n", p);
    return -1;
  }
...
void insert(Tree* t, Program* p, double number) {
  ...
  if (t -> left == NULL && t -> right == NULL)    /* If both linked nodes are empty, fill them*/
   {
       t -> left  = init_tree(copy(t->co));       /* Initialise node using a copy of previous coordinates */
       t-> left -> co-> angle -= M_PI/8;          /* Offset the angle equidistant either side */
       find_coord(t->left->co, number);           /* Offset the angle equidistant either side */
       if(p -> valgrind == false) {               /* Draw line */
   ...
   insert(t -> right, p, number);                  /* Recurse through tree until all bottom nodes are full*/
...
Coordinates* copy(Coordinates* old) {
     /*Copies were necessary as there are many coordinates, changing caused issues with freeing memory*/
